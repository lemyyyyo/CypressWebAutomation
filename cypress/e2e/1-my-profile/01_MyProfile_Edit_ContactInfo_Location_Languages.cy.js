...
describe('E2E Test: User Profile Editing', () => {
    ...
    it('should log in, edit profile, add languages, and log out', () => {
        ...
        cy.get('.edit-div').find('svg').click();

        // Click other languages dropdown
        cy.get('.sui-c-floating-label-dropdown').click();

        // Type another language
        cy.get('.sui-c-floating-label-dropdown input').type('Arabic (Chad)');

        // Click save
        cy.get('button[type="submit"]').contains('Save').click();

        // Check if other languages is updated
        cy.get('p.sui-text-b3.tw-font-bold.sui-text-darkGray-darker').should('not.contain', 'Other languages');
